...
controllers.forEach((c) => {
  ...
  // Standard three.js pattern for reliably knowing which physical hand each
  // controller Group represents — tag it from the WebXR connect event
  // itself, rather than trying to re-derive it later by cross-referencing
  // array indices (that indirection was the source of earlier bugs here).
  c.addEventListener('connected', (e) => { c.userData.xrInputSource = e.data; });
  c.addEventListener('disconnected', () => { c.userData.xrInputSource = null; });
});
function getRightController() {
  for (const c of controllers) {
    if (c.userData.xrInputSource && c.userData.xrInputSource.handedness === 'right') return c;
  }
  return null;
}
...
function drawVRSettingsPanel() {
  ...
  const bw = VR_BUTTON.x1 - VR_BUTTON.x0, bh = VR_BUTTON.y1 - VR_BUTTON.y0;
  ctx.fillStyle = 'rgba(143,214,255,0.16)';
  roundRect(ctx, VR_BUTTON.x0, VR_BUTTON.y0, bw, bh, 14);
  ctx.fill();
  ctx.strokeStyle = 'rgba(143,214,255,0.8)';
  ctx.lineWidth = 2;
  roundRect(ctx, VR_BUTTON.x0, VR_BUTTON.y0, bw, bh, 14);
  ctx.stroke();
  ctx.textAlign = 'center';
  ...
  ctx.fillText('SWITCH SHIP', canvas.width / 2, VR_BUTTON.y0 + 28);
  ctx.fillStyle = '#ffcf39';
  ctx.font = '600 18px "Segoe UI", sans-serif';
  const nextLabel = shipsReady ? SHIPS[SHIP_ORDER[(SHIP_ORDER.indexOf(currentShipKey) + 1) % SHIP_ORDER.length]].label : '...';
  ctx.fillText(`Next: ${nextLabel}`, canvas.width / 2, VR_BUTTON.y0 + 52);
...
function setVRSettingsOpen(open) {
  ...
  if (open) drawVRSettingsPanel(); // refresh in case the ship changed since it was last open
}
...
function hitToPanelAction(hit) {
  if (!hit || !hit.uv) return null;
  const px = hit.uv.x * VR_PANEL_W;
  const py = (1 - hit.uv.y) * VR_PANEL_H;
  if (Math.abs(py - VR_TRACK.musicY) <= VR_TRACK.rowHalfHeight) {
    return { type: 'slider', row: 'music', value: THREE.MathUtils.clamp((px - VR_TRACK.x0) / VR_TRACK.width, 0, 1) };
  }
...
let prevPanelTrigger = false;
function updateVRSettingsInteraction() {
  if (!renderer.xr.getSession()) { prevPanelTrigger = false; return; }

  const squeeze = getXRSqueeze();
  if (squeeze && !prevSqueeze) setVRSettingsOpen(!vrSettingsOpen);
  prevSqueeze = squeeze;

  if (!vrSettingsOpen) { prevPanelTrigger = false; return; }

  const trig = getXRTrigger();
  const rightC = getRightController();
  const action = rightC ? hitToPanelAction(raycastSettingsPanel(rightC)) : null;
  if (trig && action) {
  ...
  prevPanelTrigger = trig;
}

// ---------- XR live diagnostics overlay -------------------------------------
// Given repeated real-hardware control issues that are impossible to
// reproduce without a headset, this shows the actual raw values the game is
// reading from the controllers in real time, so problems can be diagnosed
// from what the player sees instead of guessing. Always visible in VR.
const xrDebug = makeCanvasPanel(1.0, 0.62, 460, 280);
xrDebug.mesh.position.set(-0.95, -0.35, -1.55);
xrDebug.mesh.rotation.y = 0.35;
xrDebug.mesh.renderOrder = 999;
camera.add(xrDebug.mesh);
xrDebug.mesh.visible = false;

function fmt(n) { return (n >= 0 ? ' ' : '') + n.toFixed(2); }
function drawXRDebug() {
  const session = renderer.xr.getSession();
  const { ctx, canvas, tex } = xrDebug;
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  let y = 26;
  ctx.fillText('XR DEBUG', 14, y); y += 24;

  ctx.font = '400 15px monospace';
  ctx.fillStyle = '#eaf6ff';
  if (!session) {
    ctx.fillText('no XR session', 14, y);
    tex.needsUpdate = true;
    return;
  }
  const sources = session.inputSources;
  ctx.fillText(`inputSources: ${sources.length}`, 14, y); y += 20;
  for (let i = 0; i < sources.length; i++) {
    const s = sources[i];
    ctx.fillText(`[${i}] hand=${s.handedness || '?'} gp=${s.gamepad ? 'yes' : 'no'}`, 14, y); y += 18;
  }
  y += 4;
  const rightC = getRightController();
  ctx.fillText(`getRightController: ${rightC ? 'found' : 'NULL'}`, 14, y); y += 20;

  const gp = getRightGamepad();
  if (gp) {
    const axStr = Array.from(gp.axes).map((v) => fmt(v)).join(' ');
    ctx.fillText(`R axes[${gp.axes.length}]: ${axStr}`, 14, y); y += 18;
    const btnStr = Array.from(gp.buttons).map((b, i) => `${i}:${b.pressed ? '1' : '0'}`).join(' ');
    ctx.fillText(`R btns: ${btnStr}`, 14, y); y += 18;
  } else {
    ctx.fillText('R gamepad: NULL', 14, y); y += 18;
  }

  const steer = getXRSteering();
  ctx.fillText(`steer x,y: ${steer ? fmt(steer.x) + ',' + fmt(steer.y) : 'null'}`, 14, y); y += 18;
  const aim = getXRAimDirection();
  ctx.fillText(`aim x,y,z: ${aim ? fmt(aim.x) + ',' + fmt(aim.y) + ',' + fmt(aim.z) : 'null'}`, 14, y); y += 18;
...
// All input below is restricted to the right controller only, using the
// reliable getRightController() tagged from the WebXR 'connected' event
// (see setup above) rather than any array-index guessing.
function getRightGamepad() {
  const c = getRightController();
  return c && c.userData.xrInputSource ? c.userData.xrInputSource.gamepad : null;
}

// Steering: the right thumbstick. Different browsers/controller profiles
// don't all agree on which axes[] index the stick lives at, so this tries
// the common convention (axes[2],[3]) and falls back to axes[0],[1] if that
// pair reads as idle but the other doesn't.
function getXRSteering() {
  const gp = getRightGamepad();
  if (!gp || !gp.axes) return null;
  const a = gp.axes;
  let x = a.length >= 4 ? a[2] : (a.length >= 1 ? a[0] : 0);
  let y = a.length >= 4 ? a[3] : (a.length >= 2 ? a[1] : 0);
  if (Math.abs(x) < 0.05 && Math.abs(y) < 0.05 && a.length >= 2 && (Math.abs(a[0]) >= 0.05 || Math.abs(a[1]) >= 0.05)) {
    x = a[0]; y = a[1];
  }
  return { x: THREE.MathUtils.clamp(x, -1, 1), y: THREE.MathUtils.clamp(-y, -1, 1) };
}

// Aiming: decoupled from steering — wherever the right controller is
// physically pointed, read off its tracked pose (matrixWorld), same
// mechanism the settings-panel raycaster already relies on successfully.
const _steerForward = new THREE.Vector3();
const _steerMat = new THREE.Matrix4();
function controllerForward(controller) {
  _steerMat.identity().extractRotation(controller.matrixWorld);
  return _steerForward.set(0, 0, -1).applyMatrix4(_steerMat).clone();
}
function getXRAimDirection() {
  const c = getRightController();
  if (!c) return null;
  return controllerForward(c);
}

function getXRTrigger() {
  const gp = getRightGamepad();
  return !!(gp && gp.buttons[0] && gp.buttons[0].pressed);
}
function getXRSqueeze() {
  const gp = getRightGamepad();
  return !!(gp && gp.buttons[1] && gp.buttons[1].pressed);
}
// Best-effort physical-button ship swap: try every plausible button index
// on the right controller specifically (thumbstick-click/A/B), since the
// exact index varies by browser. The in-panel SWITCH SHIP button (see
// updateVRSettingsInteraction) is the guaranteed-reliable way to do this;
// this is just a bonus shortcut.
function getXRAnyExtraButton() {
  const gp = getRightGamepad();
  if (!gp) return false;
...
function updateShipControl(dt) {
  ...
  // XR trigger fires while playing, aimed wherever the right controller is
  // physically pointing (decoupled from steering — menu confirm and
  // settings-panel dragging are handled separately, and the settings panel
  // takes priority over shooting)
  const trig = getXRTrigger();
  if (trig && !prevTrigger && !vrSettingsOpen) {
    fireProjectile(getXRAimDirection());
  }
  prevTrigger = trig;

  // Best-effort physical-button ship swap (bonus shortcut — the in-panel
  // SWITCH SHIP button, opened with grip, is the reliable way to do this).
  const extraBtn = getXRAnyExtraButton();
  if (extraBtn && !prevExtraButton && !vrSettingsOpen) switchShip(1);
  prevExtraButton = extraBtn;
}
