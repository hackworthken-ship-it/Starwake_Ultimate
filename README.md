...
- **Quest 3:** Everything uses the **right controller only**. **Right
  thumbstick** steers (push it the direction you want to fly). **Trigger**
  shoots toward wherever the controller is physically pointed — separate
  from steering, so you can fly one way while aiming another — and also
  confirms menus. **Grip** opens the in-VR **Sound & Ship** panel — point
  at it and hold the trigger to drag the volume sliders, or tap the
  **SWITCH SHIP** button to cycle ships.

If any of the right-controller input still isn't registering, a small
**XR DEBUG** panel is always visible in the lower-left of the VR view
showing exactly what the game is reading from your controllers in real
time (which hands are connected, raw thumbstick/button values, computed
steer/aim vectors) — screenshot or describe what it shows if something's
still off, since that's the fastest way to pin down what's actually
happening on real hardware.

## Switching ships

In VR, this is the reliable way: open the panel with **grip**, then point
and tap **SWITCH SHIP** (the button shows which ship is coming up next).
On desktop, press **C**. Either cycles Rocket → Transport Shuttle → E-45
Aircraft → Intergalactic Spaceship → back to Rocket, with a "NOW FLYING"
notice confirming the swap. Each ship has its own size, engine glow, and
exhaust trail tuned individually.
